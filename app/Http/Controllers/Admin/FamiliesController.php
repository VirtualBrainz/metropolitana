...
class FamiliesController extends Controller
{
    ...
    public function getHome()
    {
        ...
        $cats               = Family::orderBy('name', 'ASC')->get();
        $data = ['cats' => $cats, 'clasi' => $clasi, 'subarea' => $subarea, 'subclasi' => $subclasi, 'warehouses' => $wareh];
        return view('admin.families.home', $data);

    }

    public function postFamilyAdd(Request $request)
    {
        $rules = [
            'name'                              => 'required',
            'file'                              => 'required',
        ...
        if($validator->fails()):
            ...
            if($c->save()):
                ...
                return back()->with('message', ' Familia guardada con éxito.')->with('typealert', 'success');
    ...
    public function getFamilyEdit($id)
    {
        ...
        $kitss = DB::table('kits')->get();

        $productos = DB::table('family_products')->where('family_id', $cat->id)->get();

        $kits = DB::table('family_kits')->where('family_id', $cat->id)->get();
        
        //dd($kits);
        $data               = ['names'=> $name, 'cat' => $cat, 'clasi' => $clasi, 'subclasi' => $subclasi, 'subarea' => $subarea, 'warehouses' => $ware, 'productos' => $productos, 'kits' => $kits, 'kitss' => $kitss];
        return view('admin.families.edit', $data);
    }


    public function postFamilyEdit(Request $request, $id)
    {
        $rules = [
            'name'                              => 'required',
            'classification'                    => 'required',
        ...
        if($validator->fails()):
            ...
            if($c->save()):
                ...
                return redirect('/admin/families')->with('message', ' Familia guardada con éxito.')->with('typealert', 'success');
    ...
    public function getFamilyDelete($id)
    {
        $c = Family::find( $id);

        if($c->delete()):

            return back()->with('message', ' Familia elminada con éxito.')->with('typealert', 'success');
